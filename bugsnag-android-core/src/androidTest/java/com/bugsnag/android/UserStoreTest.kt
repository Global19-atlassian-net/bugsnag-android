...
internal class UserStoreTest {
    ...
    fun sharedPrefMigration() {
        ...
        val store = UserStore(generateConfig(true), "0asdf", file, NoopLogger, prefMigrator)
        val user = store.load().user
    ...
    fun nonExistentFile() {
        ...
        val store = UserStore(generateConfig(true), "device-id", file, NoopLogger, prefMigrator)
        val user = store.load().user
    ...
    fun nonWritableFile() {
        ...
        val store = UserStore(generateConfig(true), "device-id", nonReadableFile, NoopLogger, prefMigrator)
        val user = store.load().user
        assertEquals("device-id", user.id)
        assertNull(user.email)
        assertNull(user.name)
    ...
    fun loadWithoutPersistUser() {
        val store = UserStore(generateConfig(false), "device-id-123", file, NoopLogger, prefMigrator)
        val user = store.load().user
    ...
    fun saveWithoutPersistUser() {
        val store = UserStore(generateConfig(false), "", file, NoopLogger, prefMigrator)
