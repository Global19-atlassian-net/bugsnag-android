...
internal class UserStore @JvmOverloads constructor(
    ...
    private val previousUserHash = AtomicReference(0)
     ...
     */
    fun load(): UserState {
        val cfgUser = config.user
        val validConfigUser = validUser(cfgUser)

        val loadedUser = when {
            validConfigUser -> cfgUser
    ...
    fun save(user: User) {
        ...
        if (persist && previousUserHash.getAndSet(hashCode) != hashCode) {
            try {
